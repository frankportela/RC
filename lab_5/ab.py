...
class EntityA:
    ...
    def handle_event_wait_for_call(self, e, arg=None):
         if e == self.OUTPUT:
            ...
            print(f'EntityA: Sent packet with seqnum={p.seqnum}, waiting for ACK.')

         elif e == self.INPUT:
            if arg is not None and isinstance(arg, Msg):
              received_message = arg
              print(f'EntityA: Received message from layer5: {received_message.data}')
                    # Puedes realizar acciones adicionales según tu lógica
            else:
             print('EntityA: Unexpected input from layer5.')
    ...
    def handle_event_wait_for_ack(self, e, arg=None):
        if e == self.OUTPUT:
            ...
            if pkt_is_corrupt(p) or p.acknum != self.bit:
                print(f'EntityA: Received corrupted or unexpected ACK: {p}')
                return
            stop_timer(self)
            self.bit = 1 - self.bit
            self.handle_event = self.handle_event_wait_for_call
            self.handle_event(self.OUTPUT)
            print(f'EntityA: Received ACK for seqnum={p.seqnum}, switching to next bit.')

        elif e == self.TIMER:
            to_layer3(self, self.sent_pkt)
            start_timer(self, self.WAIT_TIME)
            print(f'EntityA: Timeout, retransmitting packet with seqnum={self.sent_pkt.seqnum}.')
...
class EntityB:
    ...
    def __init__(self, seqnum_limit=10):
        self.expecting_bit = 0
        self.seqnum_limit = seqnum_limit
        self.layer5_msgs = []  # Lista para almacenar mensajes de la capa 5
        self.sent_pkt = None
    ...
    def input(self, packet):
        ...
        if (packet.seqnum != self.expecting_bit
            ...
            pkt_insert_checksum(p)
            to_layer3(self, p)
            print(f'EntityB: Sent NAK, expecting seqnum={self.expecting_bit}.')
            ...
            print(f'EntityB: Delivered to layer5, sending ACK for seqnum={packet.seqnum}.')

    # Called when B's timer goes off.
    def timer_interrupt(self):
     last_packet = self.last_sent_packet
     to_layer3(self, last_packet)
     print(f'EntityB: Timer expired, resending last packet with seqnum={last_packet.seqnum}.')
